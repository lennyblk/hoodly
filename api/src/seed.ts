 ...
 *
 * Idempotent : users/neighbourhoods/events sont upsertés par clé stable
 * (email / name / title+neighbourhoodId) et tournent à CHAQUE déploiement
 * (voir deploy.yml) sans jamais dupliquer ni wipe l'existant.
 * Announcements/votes/conversations/messages/incidents restent des données
 * de démo one-shot, insérées uniquement au tout premier seed.
 */
...
async function seed() {
  ...
  const isFirstSeed = existingUsersCount === 0;
  console.log(
    isFirstSeed
      ? "Seed start (first run)..."
      : `DB already seeded (${existingUsersCount} users found) — syncing users/neighbourhoods/events only.`,
  );

  // ── IDs (placeholders — remplacés par le vrai _id Mongo après upsert) ────
  const uids = {
    admin: new ObjectId(),
  ...
  // ── Neighbourhoods (vrais polygones, upsert par name) ───────────────────
  const neighbourhoodDefs: { key: keyof typeof nids; doc: any }[] = [
    { key: "montmartre", doc: {
      _id: nids.montmartre,
      name: "Montmartre",
  ...
  for (const { key, doc } of neighbourhoodDefs) {
    const res = await db.collection("neighbourhoods").findOneAndUpdate(
      { name: doc.name },
      { $setOnInsert: doc },
      { upsert: true, returnDocument: "after" },
    );
    nids[key] = res!._id as ObjectId;
  }
  console.log("6 quartiers (synced)");
  ...
  const u = (key: keyof typeof uids, id: ObjectId, email: string, password: string, firstName: string, lastName: string, role: string, nid: ObjectId | null, address: string | null, points: number, days: number) => ({
    key,
    doc: {
  ...
  const userDefs = [
    u("admin", uids.admin, "admin@hoodly.com", adminPw, "Admin", "Hoodly", "admin", null, null, 50, 90),
    u("modo_montmartre", uids.modo_montmartre, "modo.montmartre@hoodly.com", modoPw, "Sophie", "Leroux", "moderateur", nids.montmartre, null, 120, 80),
    u("modo_belleville", uids.modo_belleville, "modo.belleville@hoodly.com", modoPw, "Marc", "Dupont", "moderateur", nids.belleville, null, 95, 75),
    u("modo_marais", uids.modo_marais, "modo.marais@hoodly.com", modoPw, "Nathalie", "Roche", "moderateur", nids.marais, null, 80, 70),
    u("modo_bastille", uids.modo_bastille, "modo.bastille@hoodly.com", modoPw, "Lucas", "Renard", "moderateur", nids.bastille, null, 75, 65),
    u("modo_republique", uids.modo_republique, "modo.republique@hoodly.com", modoPw, "Inès", "Garnier", "moderateur", nids.republique, null, 70, 60),
    u("modo_bonmarche", uids.modo_bonmarche, "modo.bonmarche@hoodly.com", modoPw, "Antoine", "Vidal", "moderateur", nids.bonmarche, null, 65, 55),
    // Montmartre
    u("alice", uids.alice, "alice@hoodly.com", pw, "Alice", "Martin", "habitant", nids.montmartre, "8 rue des Abbesses, 75018 Paris", 85, 60),
    u("bob", uids.bob, "bob@hoodly.com", pw, "Bob", "Bernard", "habitant", nids.montmartre, "23 rue Caulaincourt, 75018 Paris", 60, 55),
    u("julien", uids.julien, "julien@hoodly.com", pw, "Julien", "Morel", "habitant", nids.montmartre, "5 rue Gabrielle, 75018 Paris", 50, 40),
    u("camille", uids.camille, "camille@hoodly.com", pw, "Camille", "Duval", "habitant", nids.montmartre, "18 rue Lepic, 75018 Paris", 50, 35),
    // Belleville
    u("charlie", uids.charlie, "charlie@hoodly.com", pw, "Charlie", "Petit", "habitant", nids.belleville, "17 rue Denoyez, 75020 Paris", 70, 50),
    u("diana", uids.diana, "diana@hoodly.com", pw, "Diana", "Moreau", "habitant", nids.belleville, "5 rue Jouye-Rouve, 75020 Paris", 55, 45),
    u("karim", uids.karim, "karim@hoodly.com", pw, "Karim", "Bensaid", "habitant", nids.belleville, "32 rue de Belleville, 75020 Paris", 50, 38),
    u("lea", uids.lea, "lea@hoodly.com", pw, "Léa", "Fontaine", "habitant", nids.belleville, "10 rue Ramponeau, 75020 Paris", 50, 30),
    // Marais
    u("eve", uids.eve, "eve@hoodly.com", pw, "Eve", "Simon", "habitant", nids.marais, "31 rue des Francs-Bourgeois, 75004 Paris", 65, 48),
    u("frank", uids.frank, "frank@hoodly.com", pw, "Frank", "Laurent", "habitant", nids.marais, "14 rue de Rivoli, 75004 Paris", 50, 42),
    u("nadia", uids.nadia, "nadia@hoodly.com", pw, "Nadia", "Khelifi", "habitant", nids.marais, "7 rue du Temple, 75004 Paris", 50, 28),
    // Bastille
    u("grace", uids.grace, "grace@hoodly.com", pw, "Grace", "Thomas", "habitant", nids.bastille, "9 rue de la Roquette, 75011 Paris", 55, 40),
    u("hugo", uids.hugo, "hugo@hoodly.com", pw, "Hugo", "Richard", "habitant", nids.bastille, "22 rue de Charonne, 75011 Paris", 65, 35),
    u("omar", uids.omar, "omar@hoodly.com", pw, "Omar", "Diallo", "habitant", nids.bastille, "15 rue de Lappe, 75011 Paris", 50, 25),
    // Republique
    u("iris", uids.iris, "iris@hoodly.com", pw, "Iris", "Garcia", "habitant", nids.republique, "3 rue du Faubourg du Temple, 75010 Paris", 60, 32),
    u("thomas", uids.thomas, "thomas@hoodly.com", pw, "Thomas", "Lemaire", "habitant", nids.republique, "28 avenue de la République, 75011 Paris", 50, 20),
    // Bon Marche
    u("sophie", uids.sophie, "sophie@hoodly.com", pw, "Sophie", "Blanchard", "habitant", nids.bonmarche, "24 rue de Sèvres, 75007 Paris", 50, 10),
    u("remi", uids.remi, "remi@hoodly.com", pw, "Rémi", "Perrin", "habitant", nids.bonmarche, "38 rue du Bac, 75007 Paris", 50, 8),
    u("clara", uids.clara, "clara@hoodly.com", pw, "Clara", "Rousseau", "habitant", nids.bonmarche, "12 rue de Babylone, 75007 Paris", 50, 5),
  ];

  for (const { key, doc } of userDefs) {
    const res = await db.collection("users").findOneAndUpdate(
      { email: doc.email },
      { $setOnInsert: doc },
      { upsert: true, returnDocument: "after" },
    );
    uids[key] = res!._id as ObjectId;
  }
  console.log("23 utilisateurs (synced)");

  // ── Events (upsert par title + neighbourhoodId) ─────────────────────────
  const ev = (key: keyof typeof eids, id: ObjectId, title: string, desc: string, type: string, orgId: ObjectId, nid: ObjectId, date: Date, parts: ObjectId[], interest: ObjectId[], rewarded: boolean, created: number) => ({
    key,
    doc: {
  ...
  const eventDefs = [
    // Montmartre
    ev("soiree", eids.soiree, "Soirée de quartier", "Grande soirée conviviale pour tous les voisins", "social", uids.modo_montmartre, nids.montmartre, future(7), [uids.alice, uids.bob, uids.julien, uids.modo_montmartre], [uids.camille], false, 10),
    ev("jardinage", eids.jardinage, "Atelier jardinage collectif", "Venez jardiner ensemble dans le square", "atelier", uids.alice, nids.montmartre, future(14), [uids.alice, uids.camille], [uids.bob, uids.julien], false, 8),
    ev("vide_grenier", eids.vide_grenier, "Vide grenier de printemps", "Venez vendre et acheter !", "commerce", uids.modo_montmartre, nids.montmartre, past(5), [uids.alice, uids.bob, uids.julien], [], true, 20),
    ev("karaoke", eids.karaoke, "Karaoké entre voisins", "Soirée karaoké au café du coin", "social", uids.julien, nids.montmartre, future(3), [uids.julien, uids.camille, uids.bob], [uids.alice], false, 5),
    ev("cours_cuisine", eids.cours_cuisine, "Cours de cuisine du monde", "Découvrez les recettes des voisins", "atelier", uids.camille, nids.montmartre, future(21), [uids.camille, uids.alice], [uids.julien], false, 2),
    ev("rando_montmartre", eids.rando_montmartre, "Rando urbaine Montmartre", "Balade historique dans les rues secrètes", "sport", uids.alice, nids.montmartre, future(9), [uids.alice, uids.julien], [uids.camille], false, 3),
    ev("atelier_poterie", eids.atelier_poterie, "Atelier poterie", "Initiation au tournage recommandée par un voisin", "atelier", uids.bob, nids.montmartre, future(17), [uids.bob, uids.alice], [uids.julien, uids.camille], false, 2),
    ev("brunch_montmartre", eids.brunch_montmartre, "Brunch de quartier", "Brunch convivial organisé entre voisins", "social", uids.camille, nids.montmartre, future(24), [uids.camille, uids.julien], [uids.alice, uids.bob], false, 1),
    // Belleville
    ev("concert", eids.concert, "Concert de rue", "Musiciens du quartier en plein air", "culture", uids.modo_belleville, nids.belleville, future(5), [uids.charlie, uids.diana, uids.karim], [uids.lea], false, 7),
    ev("marche", eids.marche, "Marché solidaire", "Produits locaux et artisanat", "commerce", uids.modo_belleville, nids.belleville, future(10), [uids.charlie, uids.lea], [uids.diana, uids.karim], false, 6),
    ev("reunion", eids.reunion, "Réunion de quartier", "Bilan des projets en cours", "reunion", uids.modo_belleville, nids.belleville, past(3), [uids.charlie, uids.diana, uids.karim, uids.modo_belleville], [], true, 15),
    ev("fresque", eids.fresque, "Fresque murale collaborative", "Peignons ensemble le mur de la rue Denoyez", "culture", uids.karim, nids.belleville, future(18), [uids.karim, uids.lea, uids.charlie], [uids.diana], false, 4),
    ev("theatre_belleville", eids.theatre_belleville, "Théâtre de rue", "Spectacle improvisé sur la place", "culture", uids.diana, nids.belleville, future(25), [uids.diana, uids.lea], [uids.charlie], false, 2),
    ev("repair_cafe", eids.repair_cafe, "Repair Café", "Réparez vos objets avec des bénévoles", "atelier", uids.lea, nids.belleville, future(12), [uids.lea, uids.charlie], [uids.karim], false, 3),
    // Marais
    ev("expo", eids.expo, "Exposition photo", "Photos du quartier par ses habitants", "culture", uids.frank, nids.marais, future(12), [uids.frank, uids.eve, uids.nadia], [], false, 5),
    ev("yoga", eids.yoga, "Yoga en plein air", "Séance de yoga tous les samedis", "sport", uids.eve, nids.marais, future(3), [uids.eve, uids.nadia], [uids.frank], false, 4),
    ev("brocante_marais", eids.brocante_marais, "Brocante du Marais", "Antiquités et objets vintage", "commerce", uids.modo_marais, nids.marais, past(7), [uids.eve, uids.frank, uids.nadia], [], true, 18),
    ev("lecture_marais", eids.lecture_marais, "Club de lecture", "Échangez autour de vos lectures du mois", "culture", uids.nadia, nids.marais, future(8), [uids.nadia, uids.eve], [uids.frank], false, 2),
    ev("danse_marais", eids.danse_marais, "Initiation danse swing", "Apprenez le swing entre voisins", "sport", uids.frank, nids.marais, future(16), [uids.frank], [uids.eve, uids.nadia], false, 1),
    // Bastille
    ev("fete_voisins", eids.fete_voisins, "Fête des voisins", "La grande fête annuelle !", "social", uids.grace, nids.bastille, future(20), [uids.grace, uids.hugo, uids.omar], [], false, 3),
    ev("collecte", eids.collecte, "Collecte alimentaire", "Pour les familles dans le besoin", "solidarite", uids.hugo, nids.bastille, future(8), [uids.hugo, uids.omar], [uids.grace], false, 2),
    ev("course", eids.course, "Course solidaire du 11e", "5km de course pour une association", "sport", uids.omar, nids.bastille, past(2), [uids.omar, uids.hugo, uids.grace], [], true, 12),
    ev("escape_bastille", eids.escape_bastille, "Escape game de quartier", "Énigmes dans les rues de Bastille", "social", uids.grace, nids.bastille, future(13), [uids.grace, uids.hugo], [uids.omar], false, 2),
    ev("potager_bastille", eids.potager_bastille, "Potager partagé", "Entretien du potager collectif", "atelier", uids.hugo, nids.bastille, future(6), [uids.hugo], [uids.grace, uids.omar], false, 1),
    // Republique
    ev("velo", eids.velo, "Atelier réparation vélo", "Apprenez à réparer votre vélo", "atelier", uids.iris, nids.republique, future(6), [uids.iris, uids.thomas], [], false, 4),
    ev("pique_nique", eids.pique_nique, "Pique-nique de printemps", "Rendez-vous au parc !", "social", uids.thomas, nids.republique, past(10), [uids.iris, uids.thomas], [], true, 20),
    ev("debat", eids.debat, "Débat citoyen", "Discussion ouverte sur l'avenir du quartier", "reunion", uids.iris, nids.republique, future(15), [uids.iris], [uids.thomas], false, 3),
    ev("cine_republique", eids.cine_republique, "Ciné en plein air", "Projection sur la place de la République", "culture", uids.thomas, nids.republique, future(22), [uids.thomas], [uids.iris], false, 1),
    ev("tricot_republique", eids.tricot_republique, "Atelier tricot solidaire", "Tricotez des bonnets pour les sans-abri", "atelier", uids.iris, nids.republique, future(10), [uids.iris, uids.thomas], [], false, 3),
    // Bon Marche
    ev("brunch", eids.brunch, "Brunch dominical", "Brunch partagé sur la place", "social", uids.sophie, nids.bonmarche, future(4), [uids.sophie, uids.remi, uids.clara], [], false, 6),
    ev("atelier_couture", eids.atelier_couture, "Atelier couture", "Apprenez à recoudre vos vêtements", "atelier", uids.clara, nids.bonmarche, future(11), [uids.clara, uids.sophie], [uids.remi], false, 3),
    ev("marche_bm", eids.marche_bm, "Marché bio du Bon Marché", "Producteurs locaux et bio", "commerce", uids.remi, nids.bonmarche, past(4), [uids.remi, uids.sophie, uids.clara], [], true, 14),
    ev("chorale_bm", eids.chorale_bm, "Chorale de quartier", "Chantez avec vos voisins tous les mercredis", "culture", uids.sophie, nids.bonmarche, future(5), [uids.sophie, uids.clara], [uids.remi], false, 2),
    ev("lecture_bm", eids.lecture_bm, "Lecture au jardin", "Lectures à voix haute dans le jardin partagé", "culture", uids.clara, nids.bonmarche, future(17), [uids.clara], [uids.sophie, uids.remi], false, 1),
  ];

  for (const { key, doc } of eventDefs) {
    const res = await db.collection("events").findOneAndUpdate(
      { title: doc.title, neighbourhoodId: doc.neighbourhoodId },
      { $setOnInsert: doc },
      { upsert: true, returnDocument: "after" },
    );
    eids[key] = res!._id as ObjectId;
  }
  console.log(`${eventDefs.length} evenements (synced)`);

  // ── Données de démo one-shot : uniquement au tout premier seed ──────────
  if (isFirstSeed) {
    // ── Announcements ────────────────────────────────────────────────────
  ...
  // ── Neo4j (MERGE = idempotent, tourne à chaque run ; DETACH DELETE réservé au 1er seed) ──
  ...
  console.log(`${attendedPairs.length} ATTENDED (synced)`);
  ...
  console.log(`${interestedPairs.length} INTERESTED_IN (synced)`);

  await session.close();
  await driver.close();

  // ── SQLite — Incidents (données de démo one-shot) ────────────────────────
  if (isFirstSeed) {
