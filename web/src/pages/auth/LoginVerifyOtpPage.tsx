...
export default function LoginVerifyOtpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { data } = await authApi.verifySigninOtp(email, code);
      ...
      <div className="hidden lg:flex w-[420px] shrink-0 flex-col justify-between bg-vert-foret p-10">
        ...
        <div>
          <h1 className="font-heading text-4xl font-bold leading-tight tracking-tight text-white">
            Vérification<br />de sécurité.
          </h1>
          <p className="mt-4 font-sans text-sm leading-relaxed text-vert-clair/90">
            Confirmez votre identité avec le code envoyé par email pour accéder à votre compte.
          </p>
        </div>
      ...
      <div className="flex flex-1 items-center justify-center bg-creme px-6 py-10">
        <div className="w-full max-w-[420px]">
          ...
          <div className="mb-8">
            ...
            <h2 className="font-heading text-3xl font-bold tracking-tight text-charbon">Vérifiez votre identité</h2>
          ...
          <p className="mt-7 text-center font-sans text-sm text-sable">
            Mauvaise adresse ?{' '}
            <button onClick={() => navigate('/login')} className="font-semibold text-vert-moyen hover:underline">
              Recommencer
            </button>
          </p>
        </div>
